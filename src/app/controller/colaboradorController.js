...
class ColaboradorController {
  ...
  async criar(req, res) {
    try {
      ...
      res.status(200).json("usuário cadastrado com sucesso " + user.nome);
    } catch (error) {
      res
        .status(500)
        .json("não foi possivel registrar o usuário " + error.message);
    }
  }
  async usuarioUnico(req, res) {
    try {
      const id = req.params.id; // ira me retornar uma string, por isso não tinha como eu fazer a verificação com o id do modelo ja que é do tipo inteiro
      const Iduser = Number(id);

      const user = await unico(Iduser);
      res.status(200).json(user);
    } catch (error) {
      res
        .status(404)
        .json("não foi possivel encontrar o usuário " + error.message);
    }
  }
  ...
  async deletar(req, res) {
    try {
      ...
      res
        .status(500)
        .send("não foi possível deletar o usuário " + error.message);
    }
  }
